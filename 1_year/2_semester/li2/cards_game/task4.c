...
/*
                                                                                        GUIÃO 4
INPUT ESPERADO:
O programa começa lendo uma linha com o número de testes T. Para cada teste, lê duas linhas:
1. A última combinação jogada
2. A mão do jogador
A mão do jogador pode conter mais do que 14 cartas! No limite, entre a última combinação jogada e a mão do jogador, podem estar todas as cartas do baralho!

OUTPUT ESPERADO:
O programa deve imprimir o resultado dos T testes. Para cada teste deve ser impressa:
1. Uma linha com "Teste" seguida do número do teste
2. Para cada teste, deve imprimir:
   - A palavra "PASSO" ou
   - Todas as combinações melhores que tenhamos na mão
As combinações devem ser impressas uma por linha. A ordem usada deve ser a lexicográfica. As cartas de cada combinação devem ser ordenadas. As combinações devem ser ordenadas pela ordem lexicográfica das cartas.

Caso haja sequências de exceção, que podem aparecer após os Reis, estas devem aparecer depois das sequências normais. Nesses casos, devem aparecer primeiro os conjuntos e só depois as sequências duplas.
*/
...
void combSets(card cartas[], card far[], int sizec, int N, card last[], int size1, int *control) {
    ...
    for (int i = 0; i <= sizec - N; i++) {
        far[index] = cartas[i];
        combSets(cartas + i + 1, far, sizec - i - 1, N - 1, last, size1, control);
    }
}
...
void printSets(card last[], int size1, card packs[][4], int lines, int column[]) {
    ...
    for (int i = 0; i < lines; i++) {
        ...
        combSets(line, far, sizec, N, last, size1, &control);
        free(line);
    }
    ...
    for (int line = startLine; line < nLines; line++) {
        ...
        for (int col = colStart; col < nColumns[line]; col++) {
            if (!used[line]) {
                ...
                used[line] = 1;
                generateSequences(packs, nLines, nColumns, sequence, index + 1, size1, line, col + 1, used, last, control);
                used[line] = 0;
            }
        }
    }
...
void printSequences(card last[], int size1, card packs[][4], int lines, int column[]) {
    ...
    generateSequences(packs, lines, column, seq, 0, size1, 0, 0, used, last, &control);
...
void generateDoubleSequences(card cards[][4], int numLines, int *numColumns, card *sequence, int index, int sequenceSize, int startLine, int startCol, int used[], card last[], int *control, int check) {
    ...
    if (index == sequenceSize) {
        if (((seeDouble(sequence, sequenceSize) == 0) && (check == 0 || check == 2)) || ((seeDouble(sequence, sequenceSize) == 0) && (compareCards(last, sequence, sequenceSize, sequenceSize) == 0))) {
            *control = 1;
            printCombs(sequence, sequenceSize);
        }
        return;
    }
        ...
        for (int col1 = 0; col1 < numColumns[line]; col1++) {
            for (int col2 = col1 + 1; col2 < numColumns[line]; col2++) {
                if (!used[line] && (line > startLine || (line == startLine && col1 >= startCol))) {
                    ...
                    used[line] = 1;
                    generateDoubleSequences(cards, numLines, numColumns, sequence, index + 2, sequenceSize, line, col1 + 1, used, last, control, check);
                    used[line] = 0;
                }
            }
        }
...
void printDoubleSequences(card last[], int size1, card packs[][4], int lines, int column[], int check) {
    ...
    generateDoubleSequences(packs, lines, column, seq, 0, size1, 0, 0, used, last, &control, check);
...
void printKings(card last[], card packs[][4], int lines, int column[], int king, int pass) {
    ...
    if (king == 2) {
        ...
        size1 = 10;
        printDoubleSequences(last, size1, packs, lines, column, check);
    }
    ...
    if (king == 0) {
        ...
        if (var == 0) {
            ...
            printDoubleSequences(last, size1, packs, lines, column, 2);
        }
    }
}
...
void linkPrints(card last[], int size1, card packs[][4], int lines, int column[], int type) {
    ...
    if (all_King(last, size1) == 0) {
        ...
        if (king < 3) {
            ...
            switch (type) {
                case 1:
                    pass = printSetsKing(last, size1, packs, lines, column);
                    break;
                case 2: 
                    pass = printSequencesKing(last, size1, packs, lines, column); 
                    break;
                case 0:     
                    pass = printSingleKing(last, size1, packs, lines, column);
                    break;
            }
            // Imprime as cartas de 'packs' que são reis
            printKings(last, packs, lines, column, king, pass);
        }
    } else {
        ...
        switch (type) {
            case 1:
                printSets(last, size1, packs, lines, column);
                break;
            case 2: 
                printSequences(last, size1, packs, lines, column); 
                break;
            case 3: 
                printDoubleSequences(last, size1, packs, lines, column, 1); 
                break;
            case 0:     
                printSingle(last, size1, packs, lines, column);
                break;
        }
    }
}
...
int main(void) {
    ...
    int tests_number;
    wchar_t *last_string = NULL;
    wchar_t *play_string = NULL;
    ...
    if (scanf("%d", &tests_number) == 1) {
        ...
        for (int i = 0; i < tests_number; i++) {
            // Aloca memória para 'last_string' e 'play_string'
            last_string = malloc(sizeof(wchar_t) * MaxInput);
            play_string = malloc(sizeof(wchar_t) * MaxInput);

            // Lê as cartas para 'last_string' e 'play_string'
            if (scanf("%ls", last_string) == 1 && scanf("%ls", play_string) == 1) {
                // Imprime o número do teste
                printf("Teste %d\n", i + 1);
                // Processa as cartas de 'last_string' e 'play_string'
                process(last_string, play_string);
            }

            // Liberta a memória alocada para 'last_string' e 'play_string'
            free(last_string);
            free(play_string);
        }
    }    
