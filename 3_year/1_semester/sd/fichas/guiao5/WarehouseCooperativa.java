...
class WarehouseCooperativa {
    private Map<String, Product> map =  new HashMap<String, Product>();
    private ReentrantLock lock = new ReentrantLock();
    private Condition hasAllProducts = lock.newCondition();
    ...
    public void consume(Set<String> items) throws InterruptedException {
        lock.lock();
        try {
            while(hasAllProducts(items) == false)
                hasAllProducts.await();
    ...
    public boolean hasAllProducts(Set<String> items) {
        for(String s : items) {
            Product p = get(s);
            if (!p.hasQuantity(1))
                return false;
