...
def minimax(estado, profundidade, jogador):
    ...
    if profundidade == 0 or verifica_vitoria(estado, BOT) or verifica_vitoria(estado, HUMANO):
        return [-1, -1, avaliar(estado)]

    empty_cells = celulas_vazias(estado)

    for cell in empty_cells:
        x, y = cell

        aplica_movimento(x, y, jogador)

        cur_score = minimax(estado, profundidade - 1, BOT if jogador == HUMANO else HUMANO)

        estado[x][y] = 0

        cur_score[0], cur_score[1] = x, y

        if (jogador == BOT and cur_score[2] > best[2]) or (jogador == HUMANO and cur_score[2] < best[2]):
            best = cur_score
